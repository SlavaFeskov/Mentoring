...
namespace Task
{
    ...
    public class LinqSamples : SampleHarness
    {
        ...
        [Category("Task6")]
        [Title("Task 6 - 1")]
        [Description("Returns a list of customers, which have their overall orders sum above than some predefined value.")]
        public void Linq1()
        {
            var totalSums = new List<decimal> { 50000, 100000 };
            foreach (var totalSum in totalSums)
            {
                var customers = this.dataSource.Customers.Where(c => c.Orders.Sum(o => o.Total) > totalSum);
        ...
        [Category("Task6")]
        [Title("Task 6 - 5")]
        [Description("Returns a list of customers with their first order date ordered descending by year, then descending by month and then descending by orders count for that customer.")]
        public void Linq5()
        {
        ...
        [Category("Task6")]
        [Title("Task 6 - 6")]
        [Description("Returns a list of customers which have not numeric PostalCode or don't have Region or don't have phone operator (no brackets in phone number).")]
        public void Linq6()
        {
        ...
        [Category("Task6")]
        [Title("Task 6 - 7")]
        [Description("Groups products by categories, then in each group groups by presence in stock and orders by price in the last group.")]
        public void Linq7()
        {
        ...
        [Category("Task6")]
        [Title("Task 6 - 8")]
        [Description("Groups products by UnitPrice (Cheap, Medium, Expensive), the borders for each price group are predefined.")]
        public void Linq8()
        {
        ...
        [Category("Task6")]
        [Title("Task 6 - 9")]
        [Description("Returns list of cities with their average orders overall sum and average intensity of orders for each customer in the city.")]
        public void Linq9()
        {
            var result = this.dataSource.Customers.GroupBy(c => c.City).Select(
                g => new
                {
                    City = g.Key, AvgIncome = Math.Round(g.Select(c => c.Orders.Sum(o => o.Total)).Average(), 2),
                    AvgIntensity = Math.Round(g.Select(c => c.Orders.Length).Average(), 2), CustomersCount = g.Count()
                });
        ...
        public void Linq10()
        {
            var result1 = this.dataSource.Customers.SelectMany(
                c => c.Orders.GroupBy(
                    o => o.OrderDate.Year,
                    (y, o) => new { Customer = c.CompanyName, Year = y, OrdersCount = o.Count() }).OrderBy(o => o.Year));
